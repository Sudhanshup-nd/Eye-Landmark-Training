#!/usr/bin/env python3
"""
Landmark-only evaluation with NME and PCK.

Adds:
  - Per-image NME (normalization: max(bw, bh))
  - Global mean NME (average of per-image NMEs)
  - PCK at thresholds (fraction of keypoints within normalized distance <= threshold)
  - Standard per-point L2 pixel stats
...
NOTE: Updated to match new training code:
      ...
      - Resizes to config data.image_size before feeding model
      - Uses inclusive bbox width/height (+1) exactly like training dataset
"""
...
def visualize_sample(pil_img, pred_abs, gt_abs, out_png, show_gt=True):
    ...
    if show_gt and gt_abs:
        for (gx, gy) in gt_abs:
            plt.scatter([gx], [gy], c='red', s=4, marker='x')
    plt.title("Landmarks")
    plt.tight_layout()
...
def main():
    ...
    for idx, row in enumerate(tqdm(df.itertuples(index=False), total=len(df), desc="Evaluating landmarks")):
    ...
    with open(report_txt, "w") as f:
        f.write("=== Landmark Regression Evaluation ===\n")
        f.write(summary + "\n")
